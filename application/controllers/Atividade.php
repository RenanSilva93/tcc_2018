...
class Atividade extends CI_Controller {
    ...
    public function index($mensagem = NULL) {
        if ($this->session->userdata('loginuser')) {
            if ($this->session->userdata('tipo_usuario') != PROFESSOR &&
                    $this->session->userdata('tipo_usuario') != ADMINISTRADOR) {
            ...
            } else if ($this->session->userdata('tipo_usuario') == PROFESSOR) {
    ...
    public function jogarFase($nivel, $denovo = NULL) {
        if ($this->session->userdata('loginuser')) {
            ...
            $data['denovo'] = $denovo;
            if ($nivel == 1) {
                $data['perguntas'] = $this->modelAtividade->getPerguntas($nivel, $this->session->userdata('id_quiz'));
                $data['nivel'] = $nivel;
                $data['texto'] = $quiz->tema1;
                $this->load->view('atividade/quizView', $data);
            } else if ($nivel == 2) {
                $data['perguntas'] = $this->modelAtividade->getPerguntas($nivel, $this->session->userdata('id_quiz'));
                $data['nivel'] = $nivel;
                $data['texto'] = $quiz->tema2;
                $this->load->view('atividade/quizView', $data);
            } else if ($nivel == 3) {
                $data['perguntas'] = $this->modelAtividade->getPerguntas($nivel, $this->session->userdata('id_quiz'));
                $data['nivel'] = $nivel;
                $data['texto'] = $quiz->tema3;
                $this->load->view('atividade/quizView', $data);
            }
        } else {
            $this->load->view('homeView');
    ...
    public function responderQuiz() {
        if ($this->session->userdata('loginuser')) {
            ...
            while ($i < 4) {
                ...
                $data['id_quiz'] = $this->session->userdata('id_quiz');
